...
$$
L(\beta) \propto \prod_{i=1}^n \binom{m_i}{y_i} \pi(X_i)^{y_i} (1-\pi(X_i))^{m_i-y_i}
$$
...
Our hypothese for teh GOF test are:
$$
H_0: \textrm{logistic model}
$$
vs. 
$$
H_0: \textrm{saturated model}
$$
...
A third assumptions is that given predictor values, the counts of successes $Y_i$ has a binomial distribution. This means that for each case $i$
