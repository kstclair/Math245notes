...
$$
then the following is a nested model found by setting $\beta_1 = \bets_2 = 0$:
$$
...
- $SSref(log(BodyWt), danger, log(BodyWt):danger) = 343.44 + 285.01 + 15.76 = `r  343.44 + 285.01 + 15.76`$ is the regression SS for the interaction model. 


**Do we need the interaction of sleep and body weight in the model for total sleep?** I.e. are any of  the slopes for the different danger levels different?
```{r}
ggplot(sleep, aes(x=BodyWt, y=TS, color=danger)) + 
  geom_point() + 
  geom_smooth(method="lm", se=FALSE) + 
