...
```{r}
library(GGally)
ggpairs(brain, columns = c("Body","Gestation", "Litter","Brain"), lower = list(continuous = "smooth"))
```
...
plotB <- base + geom_jitter(width=.2) + labs(title="jittered")
library(gridExtra)
grid.arrange(plotA,plotB, nrow=1)
```

The `width=.2` amount specifies how much to jitter the points (we changed it here because the default amount didn't display enough of a change). We can see the jittering most in the points clustered around `Litter=0`. 
