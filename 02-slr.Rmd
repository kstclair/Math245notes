...
### Are slope and intercept estimates correlated? {#slr-simcor}
...
### Residual normal QQ plot {#slr-normality}
...
### Robustness against violations {#slr-robust}
...
#### Logarithm Practice Questions (day 6)
...
## Examples: Transformations (day 6)
...
ggplot(cars, aes(x= weight, y = city.mpg)) + 
  ...
  labs(title="City MPG vs. car weight")
```
...
We get $R^2$ in the `Multiple R-squared` entry in the `lm` `summary` output:
```{r}
summary(wood.lm)
```
Here we have $R^2 = 0.7672$, meaning the regression of depth on temp helps explain about 76.7% of the observed variation in depth. We can see how R computes this value by looking at the sample SD of $y$, $s^2_y = `r sd(wpdata$depth)`$ and the estimated model SD $\hat{\sigma} =`r summary(wood.lm)$sigma`$
```{r}
...
```{r, echo = FALSE}
plot(depth ~ temp, data=wpdata, pch=16, xlab="air temperature (C)", ylab="nest depth (cm)")
title(expression(paste("woodpeckers scatterplot: ",hat(y) == 20.12228 - 0.34218*x)))
abline(wood.lm)
abline(h=mean(wpdata$depth),col="red",lty=2)
text(24,17,expression(bar(y) == 16.3583), col="red")
# focus on case 2
text(-3,26,pos=4,expression(y == 26))
# total: y-mean(y)
# Regression: explain by regression yhat - mean(y)
yhat<- wood.lm$fitted[2]
# residual: unexplained by regression

# total = explained + unexplained
lines(c(-3,-3)-.2, c(26,mean(wpdata$depth)),lty=5,col="red",lwd=3)
lines(c(-3,-3), c(yhat,mean(wpdata$depth)),lty=5,col="blue", lwd=3)
lines(c(-3,-3), c(26,yhat),lty=5,col="green3", lwd=3)
points(-3, yhat, pch=15,col="orange")
text(-3,yhat,pos=4,expression(hat(y) == 21.14882))
legend("topright", legend=c(expression(total: y-bar(y)==9.6417 ), expression(explained: hat(y)-bar(y)==4.7905), expression(unexplained: y-hat(y)==4.8512)), lty=5, col=c("red", "blue", "green3"), lwd=3, title="Case 2 (x=-3, y=26)")
```

The next step in ANOVA, is to total the **squared** variation distances across all $n$ data points (squared because we don't care if the differences are positive or negative). Some very useful mathematics is used to prove that the total squared  variation of all $n$ cases is equal to 
$$
...
These sum of squares (SS) can also be used to compute $R^2$ since
$$
...
- **total variation:** adding the two SS values gives total variation $SST = SSreg + SSR = 179.61 + 54.60 = `r 179.61 + 54.60`$

The ANOVA SS can also give us the value of $R^2$ for the model:
$$
R^2 =  \dfrac{179.61}{179.61 + 54.60} = `r 179.61/(179.61 + 54.6)`
$$
