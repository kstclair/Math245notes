...
### R: visualizing
A scatterplot is the basic graphic that plots each $y_i$ vs. $x_i$ on the x-y coordinate plane. Suppose the data set `mydata` contains variables named `y` and `x`. The "base" R command is a simple `plot` command that looks like 
```{r, eval=FALSE}
plot(y ~ x, data= mydata)  
```
Alternatively, we could use `ggplot2` package to create a scatterplot with the `geom_point()` geometry:
```{r, eval=FALSE}
library(ggplot2)
...
Here we are interested in estimating not just a $\beta$ with confidence, but we want to estimate the **mean** response for a given value of $x_0$. E.g. We want to estimate the mean nest depth when the temp is 5 degrees. 
...
The argument `se.fit=T` provides the SE of the mean response estimate. The estimated mean depth of all nests built at 8 degrees is 17.38 cm with a SE of 0.697 cm. You can easily verify the computation of the CI:

```{r}
est <- 20.12228 -0.34218*8
est
est + c(-1,1)*qt(.975,10)*0.6972406
```
...
We can get a 95% prediction interval for the depth of one depth constructed at 8 degrees Celsius $\hat{Y} = pred(depth\mid temp=8)$  with the `predict` command with interval type `prediction` specified:
